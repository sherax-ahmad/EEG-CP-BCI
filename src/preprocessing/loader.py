...
def load_physionet(
    ...
    if data_dir:
        raws = []
        for run in runs:
            fname = Path(data_dir) / f"S{subject:03d}" / f"S{subject:03d}R{run:02d}.edf"
            if not fname.exists():
                raise FileNotFoundError(f"EDF file not found: {fname}")
            raw = mne.io.read_raw_edf(str(fname), preload=preload, verbose=False)
            raws.append(raw)
